...
public class EmployeeManagement {
    ...
    private static final Supplier<Stream<String>> supplier = () -> {
        ...
        try {
            return Files.readAllLines(Paths.get("JAVA_EX_3.2/resources/streamingvf.csv")).stream();
        } catch (IOException e) {
            throw new UncheckedIOException("Erreur lors de la lecture du fichier", e);
        }

    };

    private static final Supplier<Stream<Employee>> employees = () -> supplier.get().map(Employee::new);
    ...
    private static boolean allEmailCorrect() {
        //TODO
        return employees.get()
                .allMatch(s -> s.getEmail().endsWith("streamingvf.be"));
    }
    ...
    private static String longLastName() {
        //TODO
        return employees.get()
                .filter(employee -> employee.getLastname().length() > 14)
                .map(Employee::getFirstname)
                .findAny()
                .orElse("None");
    }
    ...
    private static Map<Boolean, List<Integer>> timeDistrubution() {
        //TODO
        return employees.get()
                .collect(Collectors.groupingBy(Employee::isFullTime,
